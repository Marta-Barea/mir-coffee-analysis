...
snv_data <- standardNormalVariate(spectral_data)

# Add corrected data to a new data frame
mir_coffee_snv <- data.frame(Varietal = mir_coffee_data$Varietal, snv_data)

# Calculate the mean for each Varietal
mir_coffee_mean <- aggregate(. ~ Varietal, mir_coffee_snv, mean)
...
snv_spectra <- ggplot(
    ...
    geom_line() +
    labs(
        title = "SNV Corrected Spectra of Instant Coffee Varietals",
...
ggsave(
    filename = file.path(plots_dir, "snv_spectra_plot.tiff"),
    plot = snv_spectra,
    width = 10,
    height = 6,
    dpi = 300
