...
# Apply Savitzky-Golay smoothing with second derivative
sgd2_data <- savitzkyGolay(
    X = spectral_data,
    p = 3,
    w = 11,
    m = 2
...
mir_coffee_sgd1 <- data.frame(Varietal = mir_coffee_data$Varietal, sgd2_data)
...
sgd2_spectra <- ggplot(
    ...
    geom_line() +
    labs(
        title = "Savitzky-Golay 2nd Derivative Corrected Spectra of Instant Coffee Varietals",
...
ggsave(
    filename = file.path(plots_dir, "sgd2_spectra_plot.tiff"),
    plot = sgd2_spectra,
    width = 10,
    height = 6,
    dpi = 300
